...
export class BusinessHealthScoreCalculator {
  ...
  private static readonly SCORE_THRESHOLDS = {
    [ScorePosition.EXCELLENT]: 85, // Updated from 80 to 85 based on top performer benchmarks
    [ScorePosition.GOOD]: 75,      // Updated from 70 to 75 based on benchmarks
    [ScorePosition.AVERAGE]: 65,   // Updated from 60 to 65 based on industry average benchmarks
    [ScorePosition.POOR]: 50,      // Maintained at 50
  ...
  private static determineScorePosition(score: number): ScorePosition {
    if (score >= this.SCORE_THRESHOLDS[ScorePosition.EXCELLENT]) {
      return ScorePosition.EXCELLENT;
    } else if (score >= this.SCORE_THRESHOLDS[ScorePosition.GOOD]) {
      return ScorePosition.GOOD;
    } else if (score >= this.SCORE_THRESHOLDS[ScorePosition.AVERAGE]) {
      return ScorePosition.AVERAGE;
    } else if (score >= this.SCORE_THRESHOLDS[ScorePosition.POOR]) {
      return ScorePosition.POOR;
    } else {
